...
Adafruit_ADXL345_Unified accel = Adafruit_ADXL345_Unified(1);//set ID for sensor
//I2C communication
...
void sensor_init() {
    ...
    if (!accel.begin()) {//check if sensor is connected
...
void sensor_read(float &x, float &y, float &z) {
    ...
    accel.getEvent(&event);//detect data from the sensor

    Serial.print("X: "); Serial.print(event.acceleration.x);//print axis data
    Serial.print("Y: "); Serial.print(event.acceleration.y);
    Serial.print("Z: "); Serial.println(event.acceleration.z);

    delay(19); // ~52 Hz sampling (1000ms / 52 ≈ 19.2ms)
}
