                                              ...
                                              int16_t scaleQ10)
{
    ...
    // 1) Mean vector (tilt/bias tolerant)
    ...
    // 2) Build 3D motion signal: |AC|^2  (3D requirement satisfied)
    for (size_t i = 0; i < N; ++i) {
        float s = 0.0f;
        if (i < nSamples) {
    ...
    float band37 = 0.0f;
    float slow = 0.0f;     // <2 Hz bucket (tilt/walk/handling)
    float total = 0.0f;    // total power
    ...
    // --- Pattern-first rhythmic gate (sensitive) ---
    // No absolute BAND37_MIN (REST gate handles noise-only windows).
    const float DOM_RATIO_MIN   = 0.12f;  // sensitive
    const float PEAKINESS_MIN   = 1.00f;  // allow modest peaks
    const float BAND_SHARE_MIN  = 0.05f;  // 3–7 Hz must be >= 6% of total
    const float SLOW_REJECT_MUL = 0.65f;  // reject strong tilt/walk (<2Hz dominates)
